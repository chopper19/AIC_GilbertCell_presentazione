...
\begin{frame}
\frametitle{Gilbert Cell design (by hand)}
\begin{enumerate}
\item Bottom-up design
\item Level 1 model
\item Results: affected by short channel effects. led to redesign on CAD tool, but reported as follows for the sake of completeness.
\end{enumerate}
\end{frame}
...
Since LO transistors are turned on only two at a time, in each LO transistor flows also half I\textsubscript{0} when on.
...
\caption{Current split in LO stage}
\label{fig:Load}
\end{figure}
\end{columns}
\end{frame}
...
\begin{columns}
\column[]{0.5\textwidth} 
R\textsubscript{S} is chosen in order to increase stage linearity without provoking a too large drop.
\begin{gather}
R_S = 10\Omega \notag\\ 
...
\frametitle{Gilbert cell CAD design - Step 1}
	\begin{columns}[c]
		\column{0.6\textwidth}
		\begin{description}
			\item [Step 1]: \textbf{maximize $g_{m3}$}. 
		\end{description}
	To do that we \textbf{impose}:
			\begin{itemize} 
...
\begin{frame}
	...
	\begin{figure}[H]
		...
		\caption{Drain current versus gate-source voltage of the RF stage, with W\textsubscript{3} varying from 50\(\mu\)m to 500\(\mu\)m, $V_{DS3} = 1.5 V$.}
		\label{fig:W_2_id}
	\end{figure}
\end{frame}
...
\begin{frame}
	...
	\begin{figure}[H]
		...
		\caption{Transconductance versus gate-source voltage of the RF stage, with W\textsubscript{3} varying from 50\(\mu\)m to 500\(\mu\)m, $V_{DS3} = 1.5 V$.}
		\label{fig:W_2_gm}
	\end{figure}
\end{frame}
...
\begin{columns}[c]
	\column{0.6\textwidth}
	\begin{description}
		\item [Step 2]: \textbf{design the current sink}. 
	\end{description}
	we \textbf{have}:
	\begin{itemize} 
		\item voltage on R\textsubscript{S}:
		\begin{gather}
		V_{R_S}=2.9mA\cdot 10 \Omega =29 mV \notag
		\end{gather}
...
\begin{columns}[c]
	...
	\begin{description}
		\item [Step 3]: \textbf{design the mixing stage}. 
	\end{description}
...
\begin{frame}
	...
	\begin{align}
		...
		&R_4=115\Omega \nonumber
	\end{align}	
	The static power consumption is $P = 5.8mA \cdot 5V = 29mW$
	\column{0.35\textwidth}
	\begin{figure} [H]
		\centering
		\scalebox{0.6}{
